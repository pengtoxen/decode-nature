 ...
 * 抛出异常处理
 * @param string $msg 异常消息
 * @param integer $code 异常代码 默认为1
 * @throws Common\Exception\MError
 * @return Common\Exception\MError
 */
function ET($msg, $code=1, $throw=true) {
    $cls='';
    if(\Org\Eco\Request::instance()->mobile){
        $cls="\\Common\\Exception\\MError";
    }else{
        $cls="\\Common\\Exception\\AdminError";
    }
    if($throw){
        throw new $cls($msg, $code);
    }else{
 ...
 * @return string
 */
function U($url='',$vars='',$suffix=true,$domain=false,$flag=true) {
    // 解析URL
    $info   =  parse_url($url);
    ...
    if($flag){
        if(defined("URI_FLAG") && !empty(URI_FLAG)){
            $url .= '/' . URI_FLAG;
        }
    }
    $url .= '/';
