...
class Model {
    ...
    {
        \Org\Eco\DbModel::instance()->link($dbname, $keep);
        if (! is_null($keep)) {
            $this->_recover_link = !$keep;
        }
        if (empty($dbname)) {
            if ($this->_link['default']) {
                $this->db = $this->_link['default'];
            }
            $this->_recover_link = false;
        } else {
            if (! $this->_link['default'] && $this->db) {
                $this->_link['default'] = $this->db;
            }
            if (! $this->_link[$dbname]) {
                $config = C($config);
                $config['DB_NAME'] = $dbname;
                $this->_link[$dbname] = Db::getInstance($config);
            }
            $this->db = $this->_link[$dbname];
        }
        return $this;
    }
    
    protected function recoverLink()
    {
        \Org\Eco\DbModel::instance()->recoverLink();
        if ($this->_recover_link) {
            $this->link();
        }
        return $this;
    }
