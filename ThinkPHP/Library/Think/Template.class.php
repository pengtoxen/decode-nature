...
class  Template {
    ...
    public function loadTemplate ($templateFile,$prefix='') {
        ...
        $tmplContent = preg_replace("/\\\"#({\\\$[a-zA-Z0-9\\\.\\\-\\\_]+})#\\\"/is", '$1', $tmplContent);
        $tmplContent =  $this->compiler($tmplContent);
        $tmplContent = preg_replace('/<!--(\\s)*beg(\\s)*-->.*?<!--(\\s)*end(\\s)*-->/is', '', $tmplContent);
        $tmplContent = preg_replace("/\\/\\/--(\\s)*beg(\\s)*.*?--(\\s)*end/is", '', $tmplContent);
    ...
    private function parseTemplateName($templateName){
        ...
        $parseStr = preg_replace("/\\\"#({\\\$[a-zA-Z0-9\\\.\\\-\\\_]+})#\\\"/is", '$1', $parseStr);
        return $parseStr;
    }    
}
